...
async function createSession(res, userId) {
  ...
  await db.prepare('INSERT INTO sessions (id, user_id, expires_at) VALUES (?,?,?)').run(id, userId, expiresAt);
...
async function createRememberToken(res, userId) {
  ...
  await db
    .prepare('INSERT INTO remember_tokens (user_id, selector, validator_hash, expires_at) VALUES (?,?,?,?)')
...
async function clearRememberToken(req, res) {
  ...
  if (raw) {
    ...
    await db.prepare('DELETE FROM remember_tokens WHERE selector = ?').run(selector);
  }
...
async function authenticate(req, res, next) {
  try {
    const sid = req.cookies[SESSION_COOKIE];
    if (sid) {
      const session = await db.prepare('SELECT * FROM sessions WHERE id = ?').get(sid);
    ...
    if (remember) {
      ...
      const token = selector && (await db.prepare('SELECT * FROM remember_tokens WHERE selector = ?').get(selector));
      if (token && new Date(token.expires_at) > new Date() && bcrypt.compareSync(validator || '', token.validator_hash)) {
        ...
        if (user) {
          // 輪換 token,避免重放
          await db.prepare('DELETE FROM remember_tokens WHERE id = ?').run(token.id);
          await createRememberToken(res, user.id);
