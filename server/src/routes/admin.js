...
router.get('/users', (req, res) => {
  const users = db.prepare('SELECT id, username, role, display_name, must_change_password, created_at FROM users ORDER BY id').all();
  res.json(users);
});
...
router.post('/users', (req, res) => {
  ...
  const info = db
    .prepare('INSERT INTO users (username, password_hash, role, display_name, must_change_password) VALUES (?,?,?,?,1)')
...
router.delete('/users/:id', (req, res) => {
  ...
  if (target.id === req.user.id) return res.status(400).json({ error: '唔可以刪除自己而家登入緊嘅帳號' });
  if (target.role === 'admin') {
    const adminCount = db.prepare("SELECT COUNT(*) AS c FROM users WHERE role = 'admin'").get().c;
    if (adminCount <= 1) return res.status(400).json({ error: '最少要保留一個管理員帳號' });
  }
  const tx = db.transaction(() => {
    db.prepare('UPDATE lesson_sessions SET created_by = NULL WHERE created_by = ?').run(target.id);
    db.prepare('UPDATE performance_scores SET updated_by = NULL WHERE updated_by = ?').run(target.id);
    db.prepare('UPDATE lesson_scores SET updated_by = NULL WHERE updated_by = ?').run(target.id);
    db.prepare('UPDATE tests SET created_by = NULL WHERE created_by = ?').run(target.id);
...
router.post('/classes/roster-batch-import', upload.single('file'), async (req, res) => {
  ...
  const tx = db.transaction(() => {
    for (const sheet of sheets) {
      ...
      let cls = findClass.get(sheet.className);
      const created = !cls;
      if (!cls) {
        const info = insertClass.run(sheet.className);
        cls = { id: info.lastInsertRowid, name: sheet.className };
      }
      for (const s of sheet.students) insertStudent.run(cls.id, s.seatNo, s.name);
...
router.post('/classes/:id/roster', upload.single('file'), async (req, res) => {
  ...
  const insert = db.prepare('INSERT INTO students (class_id, seat_no, name) VALUES (?,?,?) ON CONFLICT(class_id, seat_no) DO UPDATE SET name = excluded.name, active = 1');
  const tx = db.transaction((rows) => {
    for (const s of rows) insert.run(classId, s.seatNo, s.name);
  });
  tx(students);
...
router.get('/classes/:id/students', (req, res) => {
  const rows = db.prepare('SELECT * FROM students WHERE class_id = ? AND active = 1 ORDER BY seat_no').all(req.params.id);
  res.json(rows);
});
...
router.post('/class-subjects', (req, res) => {
  ...
  try {
    const info = db
      .prepare('INSERT INTO class_subjects (class_id, subject_id, teacher_id, homeroom_teacher) VALUES (?,?,?,?)')
...
router.put('/class-subjects/:id', (req, res) => {
  ...
  const cs = db.prepare('SELECT * FROM class_subjects WHERE id = ?').get(req.params.id);
  if (!cs) return res.status(404).json({ error: '搵唔到記錄' });
  db.prepare('UPDATE class_subjects SET teacher_id = ?, homeroom_teacher = ? WHERE id = ?').run(
    teacherId || null,
    homeroomTeacher ?? cs.homeroom_teacher,
    req.params.id
  );
  res.json({ ok: true });
});
...
router.get('/class-subjects/:id/records', (req, res) => {
  const cs = db.prepare('SELECT * FROM class_subjects WHERE id = ?').get(req.params.id);
  if (!cs) return res.status(404).json({ error: '搵唔到記錄' });
  const data = gatherClassExportData(cs);
  const mapToObj = (map) => {
    const out = {};
...
router.get('/class-subjects/:id/export', async (req, res) => {
  const cs = db.prepare('SELECT * FROM class_subjects WHERE id = ?').get(req.params.id);
  if (!cs) return res.status(404).json({ error: '搵唔到記錄' });
  const exportData = gatherClassExportData(cs);
